...
import type { CustomerHooks } from "./customer.hooks"
...
/**
 * Set hooks for Customer routes
 */
export function setCustomerHooks(h: CustomerHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "plantId", "plantName", "status", "createdAt", "updatedAt"] as const
...
export function createCustomerRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Customer not found" } }, 404)
      }
