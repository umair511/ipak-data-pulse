...
/**
 * Auto-generated ExportQuantity Routes
 *
...
import type { ExportQuantityHooks } from "./export-quantity.hooks"
...
/**
 * Set hooks for ExportQuantity routes
 */
export function setExportQuantityHooks(h: ExportQuantityHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["exportDate", "plantId", "plantName", "filmCodeId", "filmCodeName", "exportQuantityTons", "createdByName", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["filmCode"] as const
...
/**
 * Create ExportQuantity CRUD routes
 */
export function createExportQuantityRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        prisma.exportQuantity.findMany({
          where,
          include,
          orderBy,
        ...
        prisma.exportQuantity.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[ExportQuantity] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.exportQuantity.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ExportQuantity not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ExportQuantity] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.exportQuantity.create({
        data: picked.data,
      })
      ...
      console.error("[ExportQuantity] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.exportQuantity.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ExportQuantity] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.exportQuantity.delete({
        where: { id },
      })
      ...
      console.error("[ExportQuantity] Delete error:", error)
