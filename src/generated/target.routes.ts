...
import type { TargetHooks } from "./target.hooks"
...
/**
 * Set hooks for Target routes
 */
export function setTargetHooks(h: TargetHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["targetDate", "shift", "plantId", "plantName", "machineId", "machineName", "dailyTargetTons", "createdByName", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["machine"] as const
...
/**
 * Create Target CRUD routes
 */
export function createTargetRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Target not found" } }, 404)
      }
