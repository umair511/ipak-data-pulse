...
/**
 * Auto-generated DowntimeReason Routes
 *
...
import type { DowntimeReasonHooks } from "./downtime-reason.hooks"
...
/**
 * Set hooks for DowntimeReason routes
 */
export function setDowntimeReasonHooks(h: DowntimeReasonHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["reasonLabel", "status", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["productionEntries"] as const
...
/**
 * Create DowntimeReason CRUD routes
 */
export function createDowntimeReasonRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.downtimeReason.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[DowntimeReason] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.downtimeReason.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "DowntimeReason not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[DowntimeReason] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.downtimeReason.create({
        data: picked.data,
      })
      ...
      console.error("[DowntimeReason] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.downtimeReason.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[DowntimeReason] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[DowntimeReason] Delete error:", error)
