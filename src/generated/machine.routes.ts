...
import type { MachineHooks } from "./machine.hooks"
...
/**
 * Set hooks for Machine routes
 */
export function setMachineHooks(h: MachineHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["machineName", "plantId", "plantName", "section", "status", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["productionEntries", "targets"] as const
...
/**
 * Create Machine CRUD routes
 */
export function createMachineRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Machine not found" } }, 404)
      }
