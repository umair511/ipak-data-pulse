...
/**
 * Auto-generated Dispatch Routes
 *
...
import type { DispatchHooks } from "./dispatch.hooks"
...
/**
 * Set hooks for Dispatch routes
 */
export function setDispatchHooks(h: DispatchHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["dispatchDate", "customerName", "filmCodeId", "filmCodeName", "quantityTons", "dispatchType", "plantId", "plantName", "createdByName", "createdAt", "updatedAt"] as const
...
/**
 * Create Dispatch CRUD routes
 */
export function createDispatchRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.dispatch.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Dispatch not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.dispatch.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.dispatch.update({
        where: { id },
        data: picked.data,
      })
