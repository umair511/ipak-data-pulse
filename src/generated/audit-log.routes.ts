...
/**
 * Auto-generated AuditLog Routes
 *
...
import type { AuditLogHooks } from "./audit-log.hooks"
...
/**
 * Set hooks for AuditLog routes
 */
export function setAuditLogHooks(h: AuditLogHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "userName", "action", "module", "reportName", "exportFormat", "details", "createdAt"] as const
...
export function createAuditLogRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AuditLog not found" } }, 404)
      }
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.auditLog.update({
        where: { id },
        data: picked.data,
      })
