...
/**
 * Auto-generated PackingCost Routes
 *
...
import type { PackingCostHooks } from "./packing-cost.hooks"
...
/**
 * Set hooks for PackingCost routes
 */
export function setPackingCostHooks(h: PackingCostHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["costMonth", "plantId", "plantName", "totalProductionTons", "totalCostRs", "bomTotalCostRs", "actualTotalCostRs", "bomPackingCost", "actualPackingCost", "createdByName", "createdAt", "updatedAt"] as const
...
/**
 * Create PackingCost CRUD routes
 */
export function createPackingCostRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.packingCost.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[PackingCost] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.packingCost.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "PackingCost not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[PackingCost] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.packingCost.create({
        data: picked.data,
      })
      ...
      console.error("[PackingCost] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.packingCost.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[PackingCost] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[PackingCost] Delete error:", error)
