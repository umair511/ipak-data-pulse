...
/**
 * Auto-generated UserPlant Routes
 *
...
import type { UserPlantHooks } from "./user-plant.hooks"
...
/**
 * Set hooks for UserPlant routes
 */
export function setUserPlantHooks(h: UserPlantHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "plantId", "plantName", "createdAt"] as const
...
/**
 * Create UserPlant CRUD routes
 */
export function createUserPlantRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.userPlant.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[UserPlant] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.userPlant.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "UserPlant not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[UserPlant] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.userPlant.create({
        data: picked.data,
      })
      ...
      console.error("[UserPlant] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.userPlant.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[UserPlant] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[UserPlant] Delete error:", error)
