...
/**
 * Auto-generated UserPermission Routes
 *
...
import type { UserPermissionHooks } from "./user-permission.hooks"
...
/**
 * Set hooks for UserPermission routes
 */
export function setUserPermissionHooks(h: UserPermissionHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "permission", "createdAt"] as const
...
/**
 * Create UserPermission CRUD routes
 */
export function createUserPermissionRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[UserPermission] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "UserPermission not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[UserPermission] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.userPermission.create({
        data: picked.data,
      })
      ...
      console.error("[UserPermission] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[UserPermission] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[UserPermission] Delete error:", error)
