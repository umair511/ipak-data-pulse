...
/**
 * Auto-generated FilmCode Routes
 *
...
import type { FilmCodeHooks } from "./film-code.hooks"
...
/**
 * Set hooks for FilmCode routes
 */
export function setFilmCodeHooks(h: FilmCodeHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["filmCodeName", "status", "description", "plantId", "plantName", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["productionEntries", "exportQuantities", "dispatches"] as const
...
/**
 * Create FilmCode CRUD routes
 */
export function createFilmCodeRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[FilmCode] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.filmCode.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "FilmCode not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[FilmCode] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.filmCode.create({
        data: picked.data,
      })
      ...
      console.error("[FilmCode] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.filmCode.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[FilmCode] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[FilmCode] Delete error:", error)
