...
/**
 * Auto-generated ProductionEntry Routes
 *
...
import type { ProductionEntryHooks } from "./production-entry.hooks"
...
let hooks: ProductionEntryHooks = {}
...
/**
 * Set hooks for ProductionEntry routes
 */
export function setProductionEntryHooks(h: ProductionEntryHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["entryDate", "shift", "plantId", "plantName", "section", "machineId", "machineName", "filmCodeId", "filmCodeName", "productionTons", "wasteTons", "wastePercent", "downtimeMinutes", "downtimeReasonId", "downtimeReasonLabel", "numberOfSettings", "numberOfCycles", "createdByName", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["machine", "filmCode", "downtimeReason"] as const
...
/**
 * Create ProductionEntry CRUD routes
 */
export function createProductionEntryRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[ProductionEntry] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.productionEntry.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ProductionEntry not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ProductionEntry] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.productionEntry.create({
        data: picked.data,
      })
      ...
      console.error("[ProductionEntry] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.productionEntry.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ProductionEntry] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ProductionEntry] Delete error:", error)
